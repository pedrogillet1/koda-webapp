...
async function streamLLMResponse(
  ...
  for (let attempt = 1; attempt <= MAX_RETRIES; attempt++) {
    try {
      ...
      // ❌ REMOVED: Don't send here - let caller apply format enforcement first!
      // onChunk(fixedAnswer);
      // The response will be sent by the caller AFTER format enforcement
      return fixedAnswer;
...
export async function generateAnswer(
  ...
  // ✅ FIX: Accumulate all chunks, but the FINAL chunk from generateAnswerStream
  // is the format-enforced full response that replaces all previous content.
  // We keep all chunks as backup, but use the largest one as the final response.
  const chunks: string[] = [];
  ...
  const result = await generateAnswerStream(
    ...
    conversationId,
    (chunk) => {
      // Collect all chunks - the last significant one is format-enforced
      if (chunk && chunk.trim()) {
        chunks.push(chunk);
      }
    },
  ...
  // Find the largest chunk - it's likely the format-enforced full response
  // (Format enforcement sends the complete response as one chunk)
  let fullAnswer = '';
  if (chunks.length > 0) {
  ...
  console.log(`✅ [generateAnswer] Using response (${fullAnswer.length} chars from ${chunks.length} chunks)`);
