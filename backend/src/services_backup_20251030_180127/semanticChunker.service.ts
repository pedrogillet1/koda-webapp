...
class SemanticChunkerService {
  ...
  async chunkDocument(
    ...
    // MEMORY OPTIMIZATION: Skip expensive structure detection for large documents
    // Structure detection loads entire document into memory (lines array + analysis)
    // For documents > 50KB, use simple paragraph-based chunking instead
    // This prevents 6GB+ memory crashes while still allowing embeddings to be generated
    const STRUCTURE_DETECTION_SIZE_LIMIT = 50000; // characters

    if (content.length > STRUCTURE_DETECTION_SIZE_LIMIT) {
      console.log(`⚡ Large document (${content.length} chars) - using optimized paragraph chunking`);
      console.log(`   Skipping structure detection to prevent memory exhaustion`);
      return this.chunkByParagraphs(content, { maxTokens, overlapTokens });
    }

    // Step 1: Detect document structure (only for smaller documents)
