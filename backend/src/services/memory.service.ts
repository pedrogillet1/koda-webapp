...
export async function getUserMemories(
  ...
  console.log(`=� [MEMORY] Fetching memories for user ${userId.substring(0, 8)}...${section ? ` (${section})` : ''}`);
...
export async function deleteUserMemories(
  ...
  console.log(`=� [MEMORY] Deleting memories for user ${userId.substring(0, 8)}...${section ? ` (${section})` : ''}`);
